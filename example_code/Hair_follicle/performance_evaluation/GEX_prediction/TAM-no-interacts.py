    ...
    for time in range(3):
        ...
        for epoch in range(1, 3000 + 1):
            ...
            if val_score < best_score:
                ...
                es = 0
                torch.save(model.state_dict(), './nn.best.feature6.noW/'+marker+'.'+str(time)+'.pt')
            else:
                es += 1
                ...
                if es > 300:
                    ...
                    break
        model.load_state_dict(torch.load('./nn.best.feature6.noW/'+marker+'.'+str(time)+'.pt'))
        X_test = X_test.to(device)
        output = model(X_test)
        pearsons.append(pearson_correlation(y_test.detach().cpu().numpy().flatten(), output.detach().cpu().numpy()))
        
    with open("./pearson.testset.feature6_noW.txt","a") as file:
        file.write(marker+'\t')

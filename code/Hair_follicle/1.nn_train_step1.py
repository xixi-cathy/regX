...
# 1. mm10 reference genome "mm10.fa" from the UCSC genome browser (https://hgdownload.soe.ucsc.edu/goldenPath/mm10/bigZips/latest/mm10.fa.gz)
# 2. Processed genomic annotation file "mm10_geneanno.txt" from the "data" folder in this github repository
# 3. PFM mtrices of motifs obtained from "0.process_pfm_from_JASPAR.py"
# 4. Aggregated RNA and ATAC data, DE genes obtained from "0.preprocess_HairFollicle.R"
...
genes_y = df_y.columns.to_list()        
genes_notmarkers = [i for i in genes if i not in tfs_kept]
genes_notmarkers = [i for i in genes_notmarkers if i in genes_y]
print(len(genes_notmarkers))
...
def test(model, device, test_loader):
    ...
    with torch.no_grad():
        ...
        for data, target in test_loader:
            target_all.append([j for j in target.detach().cpu().numpy().tolist()])
            data, target = data.to(device), target.to(device)
            output = model(data)
            output_all.append([j for i in output.detach().cpu().numpy().tolist() for j in i])
        output_all = np.array([j for i in output_all for j in i])
        target_all = np.array([j for i in target_all for j in i])
        score = mean_squared_error(target_all, output_all)
...
test_genes = []
test_pearsons = []
for marker in genes_notmarkers:
    length = 250000
    ...
    for time in range(3):
        ...
        for epoch in range(1, 3000 + 1):
            ...
            if val_score < best_score:
                ...
                es = 0
                torch.save(model.state_dict(), './nn.best.feature6.learnW_go/'+marker+'.'+str(time)+'.pt')
            else:
                es += 1
                ...
                if es > 300:
                    ...
                    break
        model.load_state_dict(torch.load('./nn.best.feature6.learnW_go/'+marker+'.'+str(time)+'.pt'))
        X_test = X_test.to(device)
        output = model(X_test)
        pearsons.append(pearson_correlation(y_test.detach().cpu().numpy().flatten(), output.detach().cpu().numpy()))
        
    with open("./pearson.testset.feature6_learnW_go.txt","a") as file:
        file.write(marker+'\t')

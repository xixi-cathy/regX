...
parser.add_argument('--top', type=int, default=500, help='Number of driver cCREs between each two states in the output')
...
### in silico opening
if os.path.exists(args.savepath+'processing/'+args.model+'_cCRE_open.npy'):
    print("\nin silico opening of cCREs have been performed.")
    ...
    results = np.load(args.savepath+'processing/'+args.model+'_cCRE_open.npy')
else:
    print("\nPerforming in silico opening of cCREs...")
    ...
    np.save(args.savepath+'processing/'+args.model+'_cCRE_open.npy', results)
    np.save(args.savepath+'processing/'+args.model+'_cCRE_open_orig.npy', results_orig)
    np.save(args.savepath+'processing/'+args.model+'_cCRE_open_perturb.npy', results_perturb)
...
for j in range(len(start)):
    ...
    for rep in range(repeats):
        ...
        rank = ss.rankdata(np.array([np.sum(cells[:, i]) for i in range(X_test.shape[2])]))
        rank_all.append(rank)

    rank_all = np.array(rank_all)
    top = args.top
    top_cres_ind = np.argsort(np.mean(rank_all, axis=0))[::-1][:top]
    inds.append(top_cres_ind)
    top_over = [marker_peaks[i] for i in top_cres_ind]
    top_cres.append(top_over)
df_results = pd.DataFrame(top_cres).transpose()
df_results.columns = [str(start[j])+' -> '+str(end[j]) for j in range(len(start))]
df_results.to_csv(args.savepath+'results/'+args.model+'.cCRE_rank_opening.csv', index=False)
print("Done!")


print("Drawing transition plot for top "+str(top)+" cCREs between each twe states...")
if not os.path.exists(args.savepath+'results/'+args.model+'.cCRE_opening/'):
    os.makedirs(args.savepath+'results/'+args.model+'.cCRE_opening/')
rep = 0
cells = results[rep, :, :, :]
inds = pd.DataFrame([i for j in inds for i in j])[0].drop_duplicates().to_list()
for ind in inds:
    ...
    width = [abs(weight)*100 for weight in edge_weights]
    ...
    nx.draw(G, pos, with_labels=True, 
            ...
            node_size = [1000*i+1000 for i in node_size],
            width=width, 
            edge_color=edge_colors, connectionstyle="arc3,rad=0.2",
            font_size=10, font_color='black', alpha=0.9, arrowsize=25)

    plt.suptitle(cre+' (open)', fontweight='bold')
    plt.savefig(args.savepath+'results/'+args.model+'.cCRE_opening/'+cre+'.png')
    plt.show()
    plt.close()
print("Done!")    
...
### in silico closing
if os.path.exists(args.savepath+'processing/'+args.model+'_cCRE_close.npy'):
    print("\nin silico closing of cCREs have been performed.")
    model_files = [f for f in os.listdir(args.savepath + '/results/model_'+args.model+'/') if f.startswith('model_'+str(args.batchsize)+'_'+str(args.lr))]
    repeats = len(model_files)
    results = np.load(args.savepath+'processing/'+args.model+'_cCRE_close.npy')
else:
    print("\nPerforming in silico closing of cCREs...")
    print("This step takes a long time. Please be patient...")
    model_files = [f for f in os.listdir(args.savepath + '/results/model_'+args.model+'/') if f.startswith('model_'+str(args.batchsize)+'_'+str(args.lr))]
    repeats = len(model_files)
    print(str(repeats)+" replicates in total")
    ...
    np.save(args.savepath+'processing/'+args.model+'_cCRE_close.npy', results)
    np.save(args.savepath+'processing/'+args.model+'_cCRE_close_orig.npy', results_orig)
    np.save(args.savepath+'processing/'+args.model+'_cCRE_close_perturb.npy', results_perturb)
...
df_results = pd.DataFrame(top_cres).transpose()
df_results.columns = [str(start[j])+' -> '+str(end[j]) for j in range(len(start))]
df_results.to_csv(args.savepath+'results/'+args.model+'.cCRE_rank_closing.csv', index=False)
print("Done!")


print("Drawing transition plot for top "+str(top)+" cCREs between each twe states...")
if not os.path.exists(args.savepath+'results/'+args.model+'.cCRE_closing/'):
    os.makedirs(args.savepath+'results/'+args.model+'.cCRE_closing/')
rep = 0
cells = results[rep, :, :, :]
inds = pd.DataFrame([i for j in inds for i in j])[0].drop_duplicates().to_list()
for ind in inds:
    ...
    plt.suptitle(cre+' (close)', fontweight='bold')
    plt.savefig(args.savepath+'results/'+args.model+'.cCRE_closing/'+cre+'.png')
    plt.show()
    plt.close()
print("Done!")    

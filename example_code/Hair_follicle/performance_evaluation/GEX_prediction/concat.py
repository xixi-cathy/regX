...
markers = pd.read_csv("DEGs_TAC_Medulla_IRS_HS.txt", sep='\t')
...
for marker in markers:
    ...
    # X: concate peak and tf expression
    ...
    X = df_y.loc[:, tfs_use]
    X = np.array(X.iloc[cells_ind, :], dtype='float32')
    X2 = np.array(df_x.iloc[:, peaks.index], dtype='float32')
    X2 = X2[cells_ind, :]
    X = np.hstack([X2, X])
    ...
    for time in range(3):
        ...
        model = Net(num_peak=X2.shape[1], num_tf=X.shape[1]-X2.shape[1]).to(device)
        ...
        for epoch in range(1, 3000 + 1):
            ...
            if val_score < best_score:
                ...
                es = 0
                torch.save(model.state_dict(), './nn.best.ablation1.concat/'+marker+'.'+str(time)+'.pt')
            else:
                es += 1
                ...
                if es > 300:
                    ...
                    break
        model.load_state_dict(torch.load('./nn.best.ablation1.concat/'+marker+'.'+str(time)+'.pt'))
        X_test = X_test.to(device)
        output = model(X_test)
        pearsons.append(pearson_correlation(y_test.detach().cpu().numpy().flatten(), output.detach().cpu().numpy()))
        
    with open("./pearson.testset.ablation1.concat.txt","a") as file:
        file.write(marker+'\t')

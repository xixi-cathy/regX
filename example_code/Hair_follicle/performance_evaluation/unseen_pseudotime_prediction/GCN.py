...
x1 = np.where(y==3)[0]
x2 = np.where(y==2)[0]
val_test_ind = np.concatenate([x1, x2])
train_ind = [i for i in range(len(y)) if i not in val_test_ind]

X_test = X[val_test_ind, :, :]
expr_test = expr[val_test_ind, :]
y_oht_test = y_oht[val_test_ind, :]
y_test = y[val_test_ind]

X_train = X[train_ind, :, :]
expr_train = expr[train_ind, :]
y_oht_train = y_oht[train_ind, :]
y_train = y[train_ind]
...
class Net(nn.Module):
    def __init__(self, num_genes, num_peaks, num_tf, cut):
        ...
        self.out = nn.Linear(self.num_genes*self.gene_dim, 7)
...
def train(model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, expr, target) in enumerate(train_loader):
        ...
        loss = 0.7*loss_out + 0.3*loss_pseudo
        #loss = loss_pseudo
        loss.backward()
        optimizer.step()
...
edge = pd.read_csv('./predict_lineage_pseudotime/string_filtered.txt', sep='\t')
...
for rep in range(10):
    ...
    for epoch in range(1, 1000+1):
        ...
        if val_pearsonr > best_score:
            best_score = val_pearsonr
            best_test = test_pearsonr
            #best_f1 = test_score.item()
            es = 0
            torch.save(model.state_dict(), './predict_lineage_pseudotime/model.GCN.random.leaky.unseen.'+str(rep)+'.pt')
        else:
            es += 1
            ...
            if es > 100:
                break   
    print("Val: ", best_score, "Test: ", best_test)
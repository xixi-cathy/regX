...
import h5py
import math
os.chdir("/data1/xixi/scRegulate/ref_genome")


motif_files = os.listdir('./JASPAR_motifs_pfm_homosapiens')

for i in range(len(motif_files)):
    motif_file = open('./JASPAR_motifs_pfm_homosapiens/'+motif_files[i])
    flag = False
    mat = []
    ...
    np.save("./JASPAR_motifs_pfm_homosapiens/pfm.np/"+motif+".npy", mat)